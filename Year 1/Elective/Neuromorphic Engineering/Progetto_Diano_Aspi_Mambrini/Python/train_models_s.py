...
def load_spike_dataset_5ch(filepath):
    """Loads a 5-channel spike dataset (Routed or Blind) including sum.

    Expected format: 8 columns
        timestamp, label, N1, N2, N3, N4, N5, sum

    Returns a DataFrame with label + N1-N5 + sum.
    """
...
def train_and_save(data_dir, output_dir, config_label,
                   spike_prefix, neuron_cols, is_11ch):
    ...
    is_11ch : bool
        Whether the configuration uses the 11-channel dataset format.
    ...
    """
    # Select the appropriate loader based on dataset format
    load_fn = load_spike_dataset_11ch if is_11ch else load_spike_dataset_5ch
