"""
Random Forest Gesture Classifier — 5 Distance + 3 Direction Channels (1-second windows)
========================================================================================
This script:
  ...
  2. Computes Euclidean distances from the palm to each of the 5 fingertips.
     ...
     is padded with 0.
  3. Extracts the 3 direction components (direction_x, direction_y, direction_z)
     directly from the dataset.
  4. Groups samples into 1-second intervals (matching the 1 Hz UDP send rate)
     and averages the 5 distance channels AND the 3 direction channels
     within each window.
  5. Trains a Random Forest classifier on these 8 aggregated features
     (5 distances + 3 directions).
  6. Evaluates the model and saves all outputs (model, plots, scaler).
...
# ============================================================================
# 4. TIME AGGREGATION — 1-second window mean (distances + direction)
# ============================================================================
...
def aggregate_by_time_window(df: pd.DataFrame,
    ...
    5 distance channels are averaged, and the 3 direction components
    (direction_x, direction_y, direction_z) are averaged as well.
    ...
    agg : DataFrame
        ...
        direction_x_mean, direction_y_mean, direction_z_mean.
    """
    # Subselect only necessary columns to save memory (avoids OOM on 32-bit Python)
    cols_to_keep = ["timestamp", "class"] + [f"dist_{i}" for i in range(1, 6)] + DIRECTION_COLS
    ...
    agg_dict = {
        ...
        **{f"dist_{i}_mean": (f"dist_{i}", "mean") for i in range(1, 6)},
        # 3 direction means
        **{f"{col}_mean": (col, "mean") for col in DIRECTION_COLS},
...
def evaluate_model(model: RandomForestClassifier,
    ...
    fig.suptitle(
        f"Random Forest — 5 Distance + 3 Direction Channels (1s windows)\n"
        f"Accuracy: {acc * 100:.2f}%  |  Estimators: {N_ESTIMATORS}",
        fontsize=14, fontweight="bold", y=1.02,
    )
    plt.tight_layout()
    cm_path = os.path.join(output_dir, "confusion_matrix_5ch_dist_dir.png")
    fig.savefig(cm_path, dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"\n[SAVE] Confusion matrix  → {cm_path}")

    # ---- Feature importance bar chart (8 features) ----
    fig2, ax = plt.subplots(figsize=(10, 6))
    # Human-readable labels for the 8 features
    display_labels = [
        "Ch 1 (dist)", "Ch 2 (dist)", "Ch 3 (dist)",
        "Ch 4 (dist)", "Ch 5 (dist)",
        "Direction X", "Direction Y", "Direction Z",
    ]
    # Use two distinct colour palettes for distances vs directions
    dist_colors = sns.color_palette("viridis", 5)
    dir_colors = sns.color_palette("magma", 3)
    colors = dist_colors + dir_colors

    bars = ax.barh(display_labels, importances, color=colors, edgecolor="black")
    ax.set_xlabel("Importance", fontsize=12, fontweight="bold")
    ax.set_title("Feature Importance — Distance + Direction",
    ...
    plt.tight_layout()
    fi_path = os.path.join(output_dir, "feature_importance_5ch_dist_dir.png")
...
def main():
    print("=" * 70)
    print("  RANDOM FOREST — 5 DISTANCE + 3 DIRECTION CHANNELS (1s WINDOWS)")
    print("=" * 70)

    # --- Load ---
    df = load_dataset(DATASET_PATH)

    # --- Verify direction columns exist ---
    ...
    print(f"\n[INFO] Direction columns found: {DIRECTION_COLS}")
    ...
    # --- Prepare features & labels (5 distances + 3 directions = 8 total) ---
    ...
    print(f"[DATA]   → 5 distance features + 3 direction features = 8 total")
    ...
    # --- Save model ---
    model_path = os.path.join(OUTPUT_DIR, "rf_model_5ch_dist_dir.pkl")
