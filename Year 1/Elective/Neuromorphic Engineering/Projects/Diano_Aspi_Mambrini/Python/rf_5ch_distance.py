"""
Random Forest Gesture Classifier — 5 Anonymous Distance Channels
=================================================================
This script:
  1. Loads the preprocessed1.csv hand-tracking dataset.
  2. Computes Euclidean distances from the palm to each of the 5 sensor
     endpoints (ch_1 … ch_5).  These channels are treated as anonymous,
     independent features — no specific finger mapping is assumed due to
     a hardware labeling limitation.
     If data for a channel is missing (ch_X_missing == 1), its distance
     is padded with 0.
  3. Groups samples into 1-second intervals (matching the 1 Hz UDP
     prediction rate) and averages the 5 distance channels per window.
  4. Trains a Random Forest classifier on these 5 aggregated mean features.
  5. Evaluates the model and saves all outputs (model, plots).
...
Gesture classes:
    1 – Gesture 1        4 – Gesture 4
    2 – Gesture 2        5 – Gesture 5
    3 – Gesture 3        6 – Gesture 6
...
# Gesture label mapping (anonymous — no finger-specific names)
GESTURE_LABELS = {
    1: "Gesture 1",
    2: "Gesture 2",
    3: "Gesture 3",
...
# ============================================================================
# 3. FEATURE EXTRACTION — Euclidean distances (palm → channel endpoint)
# ============================================================================

def compute_distances(df: pd.DataFrame) -> pd.DataFrame:
    """
    For each of the 5 anonymous channels, compute the 3-D Euclidean
    distance from the palm centre to the channel endpoint.

    If data for a channel is missing (ch_X_missing == 1), the distance
    is zero-padded.
    ...
    for i in range(1, 6):
        endpoint_cols = [f"ch_{i}_x", f"ch_{i}_y", f"ch_{i}_z"]
        missing_col = f"ch_{i}_missing"

        # Vectorised Euclidean distance
        dx = df[endpoint_cols[0]].values - df[palm_cols[0]].values
        dy = df[endpoint_cols[1]].values - df[palm_cols[1]].values
        dz = df[endpoint_cols[2]].values - df[palm_cols[2]].values
        dist = np.sqrt(dx**2 + dy**2 + dz**2)

        # Zero-pad where the channel data is missing
        missing_mask = df[missing_col].values == 1
        dist[missing_mask] = 0.0

        df[f"dist_{i}"] = dist

    print("[FEAT] Computed 5 Euclidean-distance channels (palm → endpoint).")
...
def evaluate_model(model: RandomForestClassifier,
    ...
    fig.suptitle(
        f"Random Forest — 5 Anonymous Distance Channels (1s windows)\n"
    ...
    channel_labels = [f"Channel {i}" for i in range(1, 6)]
    colors = sns.color_palette("viridis", 5)
    bars = ax.barh(channel_labels, importances, color=colors, edgecolor="black")
    ax.set_xlabel("Importance", fontsize=12, fontweight="bold")
    ax.set_title("Feature Importance — Palm-to-Endpoint Distance (Anonymous Channels)",
...
def main():
    print("=" * 70)
    print("  RANDOM FOREST — 5 ANONYMOUS DISTANCE CHANNELS (1s WINDOWS)")
    print("=" * 70)
