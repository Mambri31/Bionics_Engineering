...
phi_o=zeros(1,H+1);
for j=1:H+1
Fact(j)=(Is*(d^2 - 2*(s(j)-s(js))^2))/(4*sigma_o*pi*(d^2 + (s(j)-s(js))^2)^(5/2));
phi_o(j)=-Is/(4*sigma_o*pi*norm([s(j)-s(js), 0, d]));
end
figure
plot(s,Fact);
...
for i=1:N %time
    for j=2:H %s -> j=1 and j=H+1 -> zero flux boundary conditions
        ...
        d2Vm=(Vm(i,j+1)-2*Vm(i,j)+Vm(i,j-1))/ds^2;
        ...
        Vm(i+1,j)=Vm(i,j)+dt/Cm *(sigma_i*a*d2Vm/2+sigma_i*a*F/2-Iion);
